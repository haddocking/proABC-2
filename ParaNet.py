...
__email__ = ["ambrosetti.francesco@gmail.com", "pierpaolo.olimpieri@gmail.com",
             "tobhe@dtu.dk", "b.jimenezgarcia@uu.nl"]


def get_features(jobid, hmmpath, light, heavy, ig_database_H, ig_database_K, ig_database_L, log_file):
    """ Calculate features necessary to run ParaNet:
    - Aligned sequences
    - Germlines
    ...
    for ab in session:
        ...
        features = {'H_HV1_len': [loop_H[list(loop_H)[0]]],
    ...
    return df_features, numbH.numbering, seq_H, numbH.aligned, numbL.numbering, numbL.aligned
...
def prediction(input_path, heavy_fasta_file, light_fasta_file):
    ...
    try:
        ...
        df, numb_h, aln_H, seq_h ,numb_L, aln_L = get_features(jobid, hmmpath, light, heavy, ig_database_H, ig_database_K,ig_database_L, log)
        ...
        seq_data = cn.encode_X_sequence(df.loc[:, ['heavy_seq', 'light_seq']], seq_encoding, add_position=1)
        ...
        try:
            os.chdir(base_dir)
            y_pred = cn.predict(tot, hps, 'ParaNet_v2', x_data)
        finally:
            os.chdir(curdir)
        ...
        print('ERROR in ParaNET prediction:')
        print(err)
        raise SystemExit()
...
if __name__ == '__main__':
    ...
    parser.add_argument('folder', help='Path to the folder containing the .fasta files for light and heavy chains')
    parser.add_argument('heavy', help='Name of the fasta file containing the heavy chain')
    parser.add_argument('light', help='Name of the fasta file containing the light chain')
