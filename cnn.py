...
def valid_X_categories():
    ...
    valid_cat = {'H_Germline':np.array(['IGHV1-Homo', 'IGHV1-Mus', 'IGHV1-Oryctolagus', 'IGHV1-Rattus', 'IGHV2-Homo', 'IGHV2-Mus', 'IGHV2-Rattus', 'IGHV3-Canis', 'IGHV3-Homo', 'IGHV3-Mus', 'IGHV3-Rattus', 'IGHV4-Homo', 'IGHV4-Mus', 'IGHV5-Homo', 'IGHV5-Mus', 'IGHV5-Rattus', 'IGHV6-Homo', 'IGHV6-Mus', 'IGHV6-Rattus', 'IGHV7-Homo', 'IGHV7-Mus', 'IGHV8-Mus', 'IGHV9-Mus', 'IGHV10-Mus', 'IGHV10-Rattus', 'IGHV12-Mus', 'IGHV14-Mus']),
                'L_Germline':np.array(['IGKV1-Homo', 'IGKV1-Mus', 'IGKV1-Oryctolagus', 'IGKV2-Homo', 'IGKV2-Mus', 'IGKV2-Rattus', 'IGKV3-Homo', 'IGKV3-Mus', 'IGKV3-Rattus', 'IGKV4-Homo', 'IGKV4-Mus', 'IGKV5-Mus', 'IGKV6-Homo', 'IGKV6-Mus', 'IGKV6-Rattus', 'IGKV7-Homo', 'IGKV8-Mus', 'IGKV8-Rattus', 'IGKV9-Mus', 'IGKV10-Mus', 'IGKV11-Mus', 'IGKV12-Mus', 'IGKV12-Rattus', 'IGKV13-Mus', 'IGKV14-Mus', 'IGKV15-Mus', 'IGKV16-Mus', 'IGKV17-Mus', 'IGKV19-Mus', 'IGKV19-Rattus', 'IGKV22-Rattus', 'IGLV1-Canis', 'IGLV1-Homo', 'IGLV1-Mus', 'IGLV2-Homo', 'IGLV3-Homo', 'IGLV3-Mus', 'IGLV3-Oryctolagus', 'IGLV3-Rattus', 'IGLV4-Homo', 'IGLV4-Rattus', 'IGLV5-Homo', 'IGLV6-Homo', 'IGLV8-Canis']),
...
def categorize_X_data(a_df):
    ...
    germ_df = pd.concat([pd.get_dummies(pd.Categorical(tmp_data['H_Germline'], categories=valid_cats['H_Germline']), prefix = 'H_G'), pd.get_dummies(pd.Categorical(tmp_data['L_Germline'], categories=valid_cats['L_Germline']), prefix = 'L_G')], axis=1, sort=False)
    H_can_df = pd.concat([pd.get_dummies(pd.Categorical(tmp_data['H_CanHV1'].astype(str), categories=valid_cats['H_CanHV1']), prefix = 'H_Can1'), pd.get_dummies(pd.Categorical(tmp_data['H_CanHV2'].astype(str), categories=valid_cats['H_CanHV2']), prefix = 'H_Can2'), pd.get_dummies(pd.Categorical(tmp_data['H_CanHV3'].astype(str), categories=valid_cats['H_CanHV3']), prefix = 'H_Can3')], axis=1, sort=False)

    L_can_df = pd.concat([pd.get_dummies(pd.Categorical(tmp_data['L_CanHV1'].astype(str), categories=valid_cats['L_CanHV1']), prefix = 'L_Can1'), pd.get_dummies(pd.Categorical(tmp_data['L_CanHV2'].astype(str), categories=valid_cats['L_CanHV2']), prefix = 'L_Can2'), pd.get_dummies(pd.Categorical(tmp_data['L_CanHV3'].astype(str), categories=valid_cats['L_CanHV3']), prefix = 'L_Can3')], axis=1, sort=False)
    HV_len_df = tmp_data.loc[:,['H_HV1_len', 'H_HV2_len', 'H_HV3_len', 'L_HV1_len', 'L_HV2_len', 'L_HV3_len']]
...
def focal_loss(gamma=4, alpha=.2, mask_val=-1):
